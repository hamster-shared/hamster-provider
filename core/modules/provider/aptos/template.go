...
//go:embed templates/aptos-docker-compose.text
...
func generateRequiredFiles() error {
	//生成genesis.blob
	genesisBlobFile, err := os.Create(filepath.Join(config.DefaultConfigDir(), "genesis.blob"))
	if err != nil {
		return err
	}
	...
	//生成waypoint.txt
	waypointFile, err := os.Create(filepath.Join(config.DefaultConfigDir(), "waypoint.txt"))
	if err != nil {
		return err
	}
	...
	publicFullNodeFile, err := os.Create(filepath.Join(config.DefaultConfigDir(), "public_full_node.yaml"))
	if err != nil {
		return err
	}
...
func templateInstance(deployParam DeployParams) error {
	t, err := template.ParseFS(templateFile, "templates/aptos-docker-compose.text")
	if err != nil {
