...
import (
	"errors"
	"github.com/hamster-shared/hamster-provider/core/modules/utils/powershell"
	"strconv"
	"strings"
)
...
$result = get-vmswitch -name $switchName  | select -First 1 | %{$_.Name}
if ($result){
   return $result
}
return $false
`
	var ps powershell.PowerShellCmd
	cmdOut, err := ps.Output(script, defaultSwitchName)

	if err != nil {
		// try to create hamster switch
		_, err = CreateVirtualSwitch(defaultSwitchName, "Internal")
	}
...
func DeleteVirtualMachine(vmName string) error {
	...
	IsRunning, err := IsRunning(vmName)
...
$hdd = Get-VMHardDiskDrive $vmName | select -First 1 | %{$_.Path}
$exists = Test-Path $hdd
if ($exists -eq "True"){
	Remove-item $hdd
} 
...
func GetVmIpAddress(vmName string) (string, error) {
	...
	for {
		...
		v[index] = mac[2*index : 2*index+2]
		index++
	}

	lowcaseMac := strings.Join(v, "-")
	arp, err := GetARP()
	if err != nil {
		return "", err
	}
	lines := strings.Split(strings.ReplaceAll(arp, "\r\n", "\n"), "\n")

	for _, str := range lines {
		if strings.Contains(str, lowcaseMac) {
			arr := strings.Fields(str)
			return arr[0], nil
		}
	}
	return "", errors.New("cannot find vm ipaddress by arp")
}
